		...
		<div class="row" style="width:1210px;margin:0 auto;">
			...
			<c:if test="${not empty pageBean && not empty pageBean.result }">
				...
				<c:if test="${vs.count==1||vs.count==7 }">
					...
					<div class="col-md-2">
						<a href="${pageContext.request.contextPath }/ProductServlet?method=findAllProductByPid&pid=${product.pid}">
							<img src="${pageContext.request.contextPath}/${product.pimage}" width="170" height="170" style="display: inline-block;">
						</a>
						<p><a href="${pageContext.request.contextPath }/ProductServlet?method=findAllProductByPid&pid=${product.pid}" style='color:green'>${product.pname }</a></p>
			...
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${not empty pageBean }">
					<c:choose>
						...
						<c:otherwise>
							<li><a href="${pageContext.request.contextPath}/ProductServlet?method=findAllProductByCidForPage&pageNumber=${pageBean.pageNumber-1}&cid=${param.cid}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
						</c:otherwise>
					</c:choose>
				</c:if>
				<!-- 当前页 -->
			...
			<c:if test="${not empty pageBean }">
					<c:forEach begin="1" end="${pageBean.totalPage}" var="i">
						<c:choose>
							...
							<c:otherwise>
								<li><a href="${pageContext.request.contextPath }/ProductServlet?method=findAllProductByCid&pageNumber=${i}&cid=${param.cid}">${i }</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</c:if>
				...
				<c:if test="${not empty pageBean }">
					<c:choose>
						...
						<c:otherwise>
							<li><a href="${pageContext.request.contextPath}/ProductServlet?method=findAllProductByCid&pageNumber=${pageBean.pageNumber+1}&cid=${param.cid}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
						</c:otherwise>
					</c:choose>
				</c:if>
			</ul>
