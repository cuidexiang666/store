	...
	<head>
	 ...
	 float:left; */
		...
		<script type="text/javascript">
			$(function(){
				var temp = $("#existItem").size();
				if(temp<=0){
					//alert("您还没有购物");
					$("#cartMsg").attr("disabled","disabled");
				}
			});
		//删除购物项
			function deleteCartItemByPid(pid){
				...
				if (confirm("您确定要删除这个商品吗？")) {
					location.href = "${pageContext.request.contextPath}/ProductServlet?method=deleteCartItemByPid&pid="+pid;
				}
			}
		//清空购物项
			window.onload = function(){
				document.getElementById("clear").onclick = function(){
					if (confirm("您确定要清空吗?")) {
						location.href = "${pageContext.request.contextPath}/ProductServlet?method=clearItem";
					}
				}
			}
		</script>
	...
	<body>
		...
		<c:choose>
			<c:when test="${empty cart.map }">
				<h3 style="text-align: center;">您还木有购物呢，赶紧去买吧！！！</h3>
			</c:when>
			<c:otherwise>
				<div class="container">
					<div class="row">
		
						<div style="margin:0 auto; margin-top:10px;width:950px;" id="cartMsg">
									<strong style="font-size:16px;margin:5px 0;">购物车详情:
											...
											<c:if test="${not empty cart && not empty cart.map }">
												<c:forEach items="${cart.map }" var="entry">
													<tr class="active" id="existItem">
														...
														<td>
															<input type="button" value="删除" class="btn btn-danger"onclick="deleteCartItemByPid('${entry.value.product.pid}')"/>
														</td>
													</tr>
												</c:forEach>
											</c:if>
							...
							<div style="margin-right:130px;">
								...
								登录后确认是否享有优惠&nbsp;&nbsp;
							</em> 赠送积分: <em style="color:#ff6600;">${cart.total}</em>&nbsp; 商品金额: <strong style="color:#ff6600;">￥${cart.total}元
								</div>
